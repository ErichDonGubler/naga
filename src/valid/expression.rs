...
impl super::Validator {
    ...
    fn global_var_ty(
        module: &crate::Module,
        function: &crate::Function,
        ty: Handle<crate::Expression>,
    ) -> Result<Handle<crate::Type>, ExpressionError> {
        use crate::TypeInner as Ti;

        match function.expressions[ty] {
            crate::Expression::GlobalVariable(var_handle) => {
                Ok(module.global_variables[var_handle].ty)
            }
            crate::Expression::FunctionArgument(i) => Ok(function.arguments[i as usize].ty),
            crate::Expression::Access { base, .. }
            | crate::Expression::AccessIndex { base, .. } => match function.expressions[base] {
                crate::Expression::GlobalVariable(var_handle) => {
                    let array_ty = module.global_variables[var_handle].ty;

                    match module.types[array_ty].inner {
                        Ti::BindingArray { base, .. } => Ok(base),
